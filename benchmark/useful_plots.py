...
def compare_model_accuracy(models:list,name:str,model_labels=list):
    ...
    fig, ax = plt.subplots(3,len(class_key),figsize=(12*len(class_key),15), gridspec_kw={'width_ratios': ratios},squeeze=False)
...
def compare_secondary_structures(model_dicts:list,name:str,model_labels=list):
    ...
    for j,model in enumerate(model_dicts):
        ...
        ax[0].bar(x=index+j*0.1, height=value_accuracy, width=0.1, align='center', color=colors[j],label=model_labels[j])
        ...
        ax[1].bar(x=index+j*0.1, height=value_recall, width=0.1, align='center', color=colors[j])
        for e,accuracy in enumerate(value_accuracy):
            ax[0].text(index[e]+j*0.1, accuracy+0.3,f"{accuracy:.3f}",ha='center', va='bottom',rotation='vertical',fontdict={'size':7})
        for e,recall in enumerate(value_recall):
            ax[1].text(index[e]+j*0.1, recall*1.2,f"{recall:.3f}",ha='center', va='bottom',rotation='vertical',fontdict={'size':7})
        #show difference
        ...
        if np.amax(difference)>maximum:
            maximum=np.amax(difference)
        ax[2].bar(x=index+j*0.1, height=difference, width=0.1, align='center', color=colors[j])
        for e,dif in enumerate(difference):
            if dif<0:
                y_coord=0
            else:
                y_coord=dif
            ax[2].text(index[e]+j*0.1, y_coord+0.01,f"{dif:.3f}", ha='center', va='bottom',rotation='vertical',fontdict={'size':7})
# Title, Label, Ticks and Ylim
        ...
        ax[2].set_xticklabels(['Helices','Sheets','Structured loops','Random'], rotation=90, fontdict={'horizontalalignment': 'center', 'size':12})
...
def plot_resolution(df: pd.DataFrame, predictions:dict, name:str, by_fragment: bool=True, ignore_uncommon=False,score_sequence=False):
    ...
    accuracy,recall=get_cath.score_each(df,predictions,by_fragment,ignore_uncommon,score_sequence)
...
def residue_plot(model,predictions,name,by_fragment=True,ignore_uncommon=False,score_sequence=False):
    fig,ax=plt.subplots(2,5,figsize=(25,10))
    sequence,prediction,dssp,true_secondary,prediction_secondary=get_cath.format_sequence(model,predictions,ignore_uncommon=ignore_uncommon,score_sequence=score_sequence,by_fragment=by_fragment)
    if not score_sequence:
        prediction=list(get_cath.most_likely_sequence(prediction))
        prediction_secondary=[list(get_cath.most_likely_sequence(ss_seq)) for ss_seq in prediction_secondary]
    ...
    ax[0][4].bar(x=index,height=seq[1],width=0.4,label='True sequence',align='center')
    ax[0][4].bar(x=index+0.4,height=pred[1],width=0.4,label='Prediction',align='center')
    ax[0][4].set_xticks(index)
    ax[0][4].set_xticklabels(pred[0], fontdict={'horizontalalignment': 'center', 'size':12})
    ...
    ax[1][4]=sns.heatmap(cm,xticklabels=seq[0],yticklabels=seq[0],square=True,cmap="viridis")
    ...
    for i,ss in enumerate(ss_names):
        ...
        ax[0][i].bar(x=index,height=seq[1],width=0.4,label='True sequence',align='center')
        ax[0][i].bar(x=index+0.4,height=pred[1],width=0.4,label='Prediction',align='center')
        ax[0][i].set_xticks(index)
        ax[0][i].set_xticklabels(pred[0], fontdict={'horizontalalignment': 'center', 'size':12})
