...
@click.option("--dataset", help="Path to .txt file with dataset list(PDB+chain, e.g., 1a2bA).",type=click.Path(exists=True),required=True)
@click.option("--training_set", help="Path to .txt file with training set, format same as dataset or PISCES",type=click.Path(exists=True),required=True)
...
@cli.command('Compare')
@click.option("--dataset", help="Path to .txt file with dataset list (PDB+chain, e.g., 1a2bA).",type=click.Path(exists=True),required=True)
@click.option("--path_to_pdb", help="Path to the directory with PDB files.",type=click.Path(exists=True),required=True)
@click.option("--path_to_assemblies", help="Path to the directory with biological assemblies.",type=click.Path(exists=True),required=True)
@click.option("--path_to_models", help="Path to the directory with .csv prediction files.",type=click.Path(exists=True),required=True)
@click.option("--path_to_dataset_map", help="Path to the .txt file with prediction labels.",type=click.Path(exists=True),required=True)
@click.option("--path_to_evoef", help="Path to the directory with EvoEF2 predictions. If supplied, EvoEF2 will be included in comparison.",type=click.Path(exists=True))
@click.option("--include", help="Path to .txt file with a list of models to be included in comparison. If not provided, 8 models with the best accuracy are compared.",type=click.Path(exists=True))
@click.option("--pdbs", help="Path to .txt file with a list of models and PDB codes for structures to be visualized. If not provided, no structures will be visualized.",type=click.Path(exists=True))
@click.option("--by_chain", is_flag=True, help="Metrics will be calculated on a full chain, otherwise only CATH fragments are considered.")
@click.option("--ignore_uncommon", is_flag=True, help="Select this option if your model ignores uncommon amino acids")
@click.option("--torsions", is_flag=True, help="Produces predicted and true Ramachandran plots for each model.")


def compare_models(dataset:str,path_to_pdb:str,path_to_assemblies:str,path_to_models:str,path_to_dataset_map:str,path_to_evoef:str=False,include:str=False,pdbs:str=False,ignore_uncommon:bool=False,by_chain:bool=False,torsions:bool=False)->None:
    ...
    for model in list_of_models:
        ...
        # make model summary
        visualization.make_model_summary(df_with_sequence,list_of_models[model],str(Path(path_to_models)/model),Path(path_to_pdb),by_fragment=by_fragment, ignore_uncommon=ignore_uncommon,score_sequence=False)
...
@click.option("--dataset", help="Path to .txt file with dataset list (PDB+chain, e.g., 1a2bA).",type=click.Path(exists=True),required=True)
@click.option("--path_to_assemblies", help="Path to the directory with biological assemblies.",type=click.Path(exists=True),required=True)
@click.option("--working_dir", help="Directory where to store results.",type=click.Path(),required=True)
@click.option("--path_to_evoef2", help="Path to EvoEF2 executable.",type=click.Path(exists=True),required=True)
@click.option("--max_processes", help="Maximum number of cores to use",type=int,default=8)

def run_evoEF2(dataset:str,working_dir:str,path_to_evoef2:str, max_processes:int,path_to_assemblies: str)->None:
